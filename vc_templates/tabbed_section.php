...
if ( false !== strpos( $content, 'pf_vc_logo_slide' ) && ! $nectar_using_VC_front_end_editor && ! is_admin() ) {
	// Color.
	$pf_active_color = '#0f95b7';

	// Content.
	$pattern          = '/\[tab [^\]]*\]/i';
	$replacement      = '';
	$content_filtered = preg_replace( $pattern, $replacement, $content );
	$content_filtered = str_replace( '[/tab]', '', $content_filtered );
	$pattern          = '/\[tabbed_section [^\]]*\]/i';
	$content_filtered = preg_replace( $pattern, $replacement, $content );
	$content_filtered = str_replace( '[/tabbed_section]', '', $content_filtered );

	$pattern_not_pf_tab = '/\[vc_(.+?)\[\/vc_(.+?)\]/i'; // this is not necessary though.
	$content_filtered   = preg_replace( $pattern_not_pf_tab, $replacement, $content_filtered );

	$pattern_empty_tab = '/\[tab[^\]]*\]\[\/tab\]/i';
	$content_filtered  = preg_replace( $pattern_empty_tab, $replacement, $content_filtered );

	$output = '';
	$html         = do_shortcode( $content_filtered );
	$pattern_link = '/<a href="#" class="logo-slider__logo.*?>(.+?)<\/a>/i';
	preg_match_all( $pattern_link, $html, $nav_links );
	$html = preg_replace( $pattern_link, $replacement, $html );

	$output .= '<div class="logo-slider" data-companies-slider>';
	$output .= '<div class="logo-slider__container">';
	$output .= '<div class="logo-slider__logos" data-companies-nav>';

	$i = 0;
	foreach ( $nav_links[1] as  $item ) {
		$active = '';
		if ( 0 === $i ) {
			$active = ' active';
		}
		$output .= '<a href="#" class="logo-slider__logo project-logo' . $active . '" data-slide-to="' . $i . '">' . $item . '</a>';
		$i++;
	}

	$output .= '</div>';

	$output .= '<div class="logo-slider__slides" data-companies-slides>';
	$output .= $html;
	$output .= '</div>';
	$output .= '</div>'; // logo-tabs__container.
	$output .= '</div>';
